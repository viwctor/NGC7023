...
- **GIF output is two-pass** for a high-quality palette without the single-pass pitfalls. `JobEngine._run_gif` runs `MediaJob.build_gif_palettegen_args()` (pass 1: editing filters → `palettegen=stats_mode=diff` → a temp PNG) then `build_gif_encode_args()` (pass 2: `-i video -i palette`, `[0:v]<filters>[x];[x][1:v]paletteuse=…[v]`, `-map "[v]"`, `-an`). The earlier single-pass `split` (`_gif_filter_complex`, still used only for the argv *preview*) buffered every paletteuse-branch frame until palettegen finished — which stalled progress at 0% and OOM'd on real-length videos. Two-pass streams pass 2 frame-by-frame: real progress, low memory. **Still-image** output carries no codec/CRF/audio (forcing `-c:v` breaks the mux). HEVC in mp4/mov gets the `hvc1` tag.
