...
class Api:
    ...
    def restart_app(self, params: dict | None = None) -> None:
        """Launch a fresh instance, then close this one shortly after. The new
        instance gets ``--relaunch`` so the single-instance guard waits for this
        one to release its port instead of deferring back to it (which would
        leave no instance running)."""
        try:
            if getattr(sys, "frozen", False):
                subprocess.Popen([sys.executable, "--relaunch"], creationflags=_CREATE_NO_WINDOW)
            else:
                subprocess.Popen(
                    [sys.executable, "-m", "ngc7023", "--relaunch"],
